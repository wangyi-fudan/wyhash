...
#    else
    uint64_t ha = A >> 32;
    uint64_t hb = B >> 32;
    uint64_t la = (uint32_t)A, lb = (uint32_t)B, hi, lo;
    uint64_t rh = ha * hb;
    uint64_t rm0 = ha * lb;
    uint64_t rm1 = hb * la;
    uint64_t rl = la * lb;
...
#    endif
#endif
static inline uint64_t _wyread3(const uint8_t *p, unsigned k) {
...
static inline uint64_t FastestHash(const void *key, size_t len) {
    ...
    if (len >= 8)
        return _wymum(_wyread8(p), _wyread8(p + len - 8));
    else if (_like_(len >= 4))
        return _wymum(_wyread4(p), _wyread4(p + len - 4));
    else if (_like_(len))
        return _wymum(_wyread3(p, len), _wyread3(p, len));
    else
        return 0;
}
...
static inline uint64_t _wyhash(const void *key, uint64_t len, uint64_t seed,
    ...
    label:
        if (_like_(i >= 8)) {
            if (_like_(i <= 16))
                return _wymum(_wyread8(p) ^ secret[0], _wyread8(p + i - 8) ^ seed);
            else if (_like_(i <= 32))
                return _wymum(_wyread8(p) ^ secret[0], _wyread8(p + 8) ^ seed) ^
                       _wymum(_wyread8(p + i - 16) ^ secret[1],
                              _wyread8(p + i - 8) ^ seed);
            else
                return _wymum(_wyread8(p) ^ secret[0], _wyread8(p + 8) ^ seed) ^
                       _wymum(_wyread8(p + 16) ^ secret[1], _wyread8(p + 24) ^ seed) ^
                       _wymum(_wyread8(p + i - 32) ^ secret[2],
                              _wyread8(p + i - 24) ^ seed) ^
                       _wymum(_wyread8(p + i - 16) ^ secret[3],
                              _wyread8(p + i - 8) ^ seed);
        } else {
            if (_like_(i >= 4))
                return _wymum(_wyread4(p) ^ secret[0], _wyread4(p + i - 4) ^ seed);
            else
                return _wymum((_like_(i) ? _wyread3(p, i) : 0) ^ secret[0], seed);
        }
    }
    uint64_t seed1 = seed, seed2 = seed, seed3 = seed;
    for (; i >= 64; i -= 64, p += 64) {
        seed = _wymum(_wyread8(p) ^ secret[0], _wyread8(p + 8) ^ seed);
        seed1 = _wymum(_wyread8(p + 16) ^ secret[1], _wyread8(p + 24) ^ seed1);
        seed2 = _wymum(_wyread8(p + 32) ^ secret[2], _wyread8(p + 40) ^ seed2);
        seed3 = _wymum(_wyread8(p + 48) ^ secret[3], _wyread8(p + 56) ^ seed3);
    }
...
static inline uint64_t wyhash64(uint64_t A, uint64_t B) {
    return _wymum(_wymum(A ^ 0xa0761d6478bd642full, B ^ 0xe7037ed1a0b428dbull),
...
struct hasher {
    size_t operator()(const string &s) const {
        return wyhash(s.c_str(), s.size(), 0);
    }
};
int main(void) {
    std::vector<string> keys(1ull << 20);
    std::vector<bool> used(1ull << 20);
    std::vector<unsigned> values(1ull << 20);
    string s;
    size_t pos = 0;
