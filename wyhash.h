...
#include <stdint.h>
#include <string.h>
const	uint64_t	_wyp0=0xb10f1ea5b4358d87ull,	_wyp1=0x2e63952eb46a7127ull;
...
static inline uint64_t wyhash(const void *key, uint64_t len, __uint128_t seed){
 const __uint128_t _wyp=(((__uint128_t)_wyp0)<<64)|_wyp1;  
 const uint8_t *p=(const uint8_t *)key;
 uint64_t len0=len; 
 for(;len>16;len-=16,p+=16) seed=_wymix128(_wyread128(p)^seed^_wyp);
 seed=_wymix128(_wymix128((_wyread128(p)<<((16-len)<<3))^seed^_wyp)^len0); 
 return (seed>>64)^(uint64_t)seed; 
}
