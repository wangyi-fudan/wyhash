...
#define _likely_(x) (x)
#endif
const uint64_t _wyp[10]={0xb10f1ea5b4358d87ull,0x2e63952eb46a7127ull,0xd88be12db28d1769ull,0xb8d2a6b2d21e6c55ull,0xe43ad88ed1c9ac1bull,0xe4a33a5336a35a1dull,0xe85aca652dd266a5ull,0x2e1d72638e6c2e4bull,0x8ba6d4726c8db48dull,0x935a749a3ae2478bull};
...
static inline uint64_t _wyhash(const void* key, uint64_t len, uint64_t seed, const uint64_t secret[10]){
 ...
 for(; i>128; i-=128,p+=128){
  ...
  for(size_t j=0; j<4; j++){
   ...
   se[j]=_mm256_xor_si256(se[j],_mm256_mul_epu32 (se[j],_mm256_shuffle_epi32(se[j],0x31)));
  }
 }
 ...
 for(; i>128; i-=128,p+=128){
  ...
  for(size_t j=0; j<8; j++){
   ...
   se[j]=_mm_xor_si128(se[j],_mm_mul_epu32 (se[j],_mm_shuffle_epi32(se[j],0x31)));
  }
 }
 for(size_t	j=0; j<8; j++){ uint64_t *s=(uint64_t*)&se[j]; seed^=s[0]^s[1];}
#else
 uint64_t se[4][4]={ {seed,seed,seed,seed}, {seed,seed,seed,seed}, {seed,seed,seed,seed}, {seed,seed,seed,seed}};
 uint64_t ma[4][4]={
  {secret[0]^secret[4],secret[0]^secret[5],secret[0]^secret[6],secret[0]^secret[7]},
  {secret[1]^secret[4],secret[1]^secret[5],secret[1]^secret[6],secret[1]^secret[7]},
  {secret[2]^secret[4],secret[2]^secret[5],secret[2]^secret[6],secret[2]^secret[7]},
